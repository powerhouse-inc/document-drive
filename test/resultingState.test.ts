...
describe('Document operations', () => {
    ...
    it("should retrieve resultingState for last unskipped operation", async ({ expect }) => {
        const driveStorage = await storage.getDrive("test");
        expect(driveStorage.operations.global.length).toBe(3);
        expect(JSON.parse(driveStorage.operations.global.at(2)?.resultingState as string)).toStrictEqual({
            "icon": null,
            "id": "test",
            "name": "",
            "nodes": [
        ...
        expect(driveStorage.operations.global.at(1)?.resultingState).toBeUndefined();
        const result = await server.addDriveOperation(
            "test",
            // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
            { ...actions.noop(), skip: 1, index: 3, hash: driveStorage.operations.global.at(1)?.hash }
        );
        ...
        expect(driveStorage.operations.global.length).toBe(3);
        expect(result.document?.operations.global.at(2)?.resultingState).toStrictEqual(resultingState);
        expect(JSON.parse(result.document?.operations.global.at(1)?.resultingState as string)).toStrictEqual(resultingState);
    })
});